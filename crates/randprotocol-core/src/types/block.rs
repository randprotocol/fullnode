...
impl Vote {
    pub fn sign(view: u64, block_hash: Hash, key: &Keypair) -> Vote {
        let signature = key.sign(&Hash::digest_domain(b"rand-vote", &vote_message(view, &block_hash)).0);
    ...
    pub fn verify(&self) -> bool {
        self.voter.verify(
            &Hash::digest_domain(b"rand-vote", &vote_message(self.view, &self.block_hash)).0,
            &self.signature,
        )
    }
}
