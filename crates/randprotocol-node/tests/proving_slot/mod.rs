...
//! (`PROVING` in `cluster.rs`): blocks slow enough that even a proof competing with six others
//! for the same cores finished inside the window. That is a margin, not a bound — it holds only
//! as long as nobody adds another proving test and nobody runs on fewer cores.
//!
...
//! second acquisition on the same thread waits for the first, which will not be released. A test
//! that deliberately proves two bundles at once (`two_bundles_spending_one_note_only_one_commits`,
//! whose whole subject is that race) takes the slot once, for the pair: the slot bounds
//! *unrelated* proofs, never a test's own deliberate concurrency.
//!
//! This file has a twin at `crates/randprotocol-client/tests/proving_slot/mod.rs`, because both test
...
fn lock_path() -> PathBuf {
    PathBuf::from(env!("CARGO_TARGET_TMPDIR")).join("rand-proving-slot.lock")
}
...
fn the_slot_is_one_path_for_the_whole_workspace() {
    ...
    assert!(path.ends_with("tmp/rand-proving-slot.lock"), "{}", path.display());
