...
//! challenger (R2), then the interface digest over `[inner_vk_digest ‖ N ‖ 34·N]` (R5).
//!
...
//! word, and each proof's thirty-four public values as its iteration accepts them — through a
//! cursor cell tracking the next rate lane's absolute address (`dsl::hash::absorb_staged`). The
//! state lives in eight *dedicated* cells: the pipeline hashes through the shared
//! `Builder::hash_scratch` tens of thousands of times per iteration, and the interface sponge
...
//! own lanes and permutes once. With the stream `[vk(4) ‖ N ‖ 34·N]`, the position after the
//! last word is `(1 + 34N) mod 4 ∈ {1, 3}` for `N ≥ 1` — never a block boundary — so the final
//! permutation is unconditional, one row, no branch.
...
pub fn verify_rv32n(shape: &InnerShape, key: &InnerKey, cp: Checkpoints) -> VerifierProgram {
    ...
    // empty aggregate is refused here, before any proof is read.
    let n = b.hint();
    b.assert_nonzero(n, "aggregate count");

    // ── the interface sponge's preamble. The state: zero lanes, the domain tag and the word
    // count `4 + 1 + 34·N` in the capacity lanes — the count computed from the tape's `N`, so
    ...
    let len = b.add_const(len, F::from_u64((DIGEST_ELEMS + 1) as u64));
    b.store(st, 5, len);
    ...
    b.note_phase("preamble: the count word, the interface state, the vk digest");
