...
/// The hash-domain tag of [`inner_vk_digest`]. `randprotocol_zkvm::notes::domain` is occupied through 14 and
...
impl InnerShape {
    ...
    /// `randprotocol_zkvm::machine::val_mmcs_for_tests`).
    #[doc(hidden)]
...
pub fn inner_vk_digest<S: VerifierShape>(shape: &S, key: &S::Key) -> [F; 4] {
    let sponge = PaddingFreeSponge::<Perm, 8, 4, 4>::new(randprotocol_zkvm::machine::permutation());
...
pub(crate) fn val_mmcs() -> &'static randprotocol_zkvm::machine::ValMmcs {
    static MMCS: std::sync::OnceLock<randprotocol_zkvm::machine::ValMmcs> = std::sync::OnceLock::new();
    MMCS.get_or_init(randprotocol_zkvm::machine::val_mmcs_for_tests)
}
...
pub(crate) fn natural_domain(
    ...
    <randprotocol_zkvm::machine::Pcs as Pcs<Challenge, randprotocol_zkvm::machine::Challenger>>::natural_domain_for_degree(
        cfg.pcs(),
        size,
    )
}
...
/// segment opens with, in the shape's own order. Implemented for `randprotocol_zkvm::machine::Proof`
