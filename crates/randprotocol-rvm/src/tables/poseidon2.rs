...
//! **shared** with `randprotocol_zkvm::tables::poseidon2`, so the chip's permutation is the machine's
...
pub use randprotocol_zkvm::tables::poseidon2::{cube, internal_matmul, mds_light, round_constants};
    ...
    for row in 0..height {
        ...
        let (clk, input, output) = match events.get(row) {
            Some(&(clk, ev)) => {
                ...
                debug_assert_eq!(
                    ev.output,
                    randprotocol_zkvm::tables::poseidon2::permute_scalar(ev.input),
