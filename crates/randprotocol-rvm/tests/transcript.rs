...
type Perm = randprotocol_zkvm::machine::Perm;
type Chal = randprotocol_zkvm::machine::Challenger;
fn perm() -> Perm {
    randprotocol_zkvm::machine::permutation()
}
fn native() -> Chal {
    Chal::new(perm())
}
...
fn sample_bits_matches_the_low_bits_of_the_canonical_representative() {
    ...
    for bits in [1usize, 8, 20, 21, 27] {
        ...
        w.push(randprotocol_rvm::dsl::transcript::canonicality_hint(v));
...
fn sample_bits_rejects_a_non_canonical_decomposition() {
    ...
    for (hint, which) in [
        ...
        (randprotocol_rvm::dsl::transcript::canonicality_hint(v), "lo's real inverse"),
    ] {
        let mut w = bits.clone();
        w.push(hint);
        let p = build();
        match execute(&p, &w, 5_000_000) {
            Err(randprotocol_rvm::emulator::ExecError::InverseOfZero { pc }) => {
...
fn sample_bits_rejects_a_forged_decomposition_of_the_sampled_element() {
    ...
    with_bit_hint.push(randprotocol_rvm::dsl::transcript::canonicality_hint(x.as_canonical_u64()));
...
fn check_witness_accepts_only_a_grinding_witness_p3_accepts() {
    ...
    for (w_val, ok) in [(witness, true), (witness + F::ONE, false)] {
        ...
        tape.push(randprotocol_rvm::dsl::transcript::canonicality_hint(v));
...
fn observe_digest_and_observe_cap_match_p3_challengers_digest_and_cap_absorbs() {
    ...
    let mut b = Builder::new(Checkpoints::Off);
    let mut ch = DslChallenger::new(&mut b);
    let dp = randprotocol_rvm::dsl::Digest(b.alloc(4));
    for (k, v) in d.iter().enumerate() {
        let h = b.constant(*v);
        b.store(dp.0, k as i64, h);
    }
    ch.observe_digest(&mut b, dp);
    let capp: [randprotocol_rvm::dsl::Digest; 4] = core::array::from_fn(|j| {
        let p = randprotocol_rvm::dsl::Digest(b.alloc(4));
...
fn the_dsl_compression_matches_truncated_permutation_2_4_8() {
    ...
    for _ in 0..20 {
        ...
        hash::compress(
            &mut b,
            randprotocol_rvm::dsl::Digest(lp),
            randprotocol_rvm::dsl::Digest(rp),
            randprotocol_rvm::dsl::Digest(op),
        );
        for k in 0..4 {
            let v = b.load(op, k);
            b.public(v);
...
fn a_restored_merkle_path_verifies_in_the_dsl_exactly_where_p3_verifies_it() {
    ...
    let mmcs = randprotocol_zkvm::machine::val_mmcs_for_tests(); // the crate's own ValMmcs, seeded
    ...
    for (q, idx) in indices.iter().enumerate() {
        ...
        let leaf = randprotocol_rvm::dsl::Digest(b.alloc(4));
        ...
        let root = randprotocol_rvm::dsl::Digest(b.alloc(4));
...
fn an_injected_shorter_matrix_group_verifies_in_the_dsl_exactly_where_p3_verifies_it() {
    ...
    use p3_matrix::dense::RowMajorMatrix;
    use p3_matrix::Dimensions;
    let mmcs = randprotocol_zkvm::machine::val_mmcs_for_tests();
