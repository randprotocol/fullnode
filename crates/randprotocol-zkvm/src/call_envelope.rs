...
//! (`randprotocol_core::ledger::call_envelope`). What ties an envelope to its call is the AEAD: the
...
//! domain tags are untouched; the ones here are distinct (`rand-call-*`), so no wrap of one
...
const AAD_SENDER: &[u8] = b"rand-call-sender";
const AAD_AUDITOR: &[u8] = b"rand-call-auditor";
