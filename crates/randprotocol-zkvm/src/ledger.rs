...
pub enum LedgerError {
    ...
    /// no value. What it would corrupt is `Ledger::fees_collected`, which is RAND (asset 0)
...
pub struct Ledger {
    ...
    /// (RAND), and `apply_bundle` refuses a bundle that declares another asset with a
    /// non-zero fee (`LedgerError::FeeInForeignAsset`), so nothing but RAND can ever reach
    /// this accumulator.
    ...
    /// has to become per-asset; do not read it as RAND until then.
    pub burned: u64,
}
