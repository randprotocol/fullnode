...
use randprotocol_zkvm::machine::{build_traces, FriProfile, Machine, Tier, TIERS};
use randprotocol_zkvm::tables::{alu, cpu, input, keccak, memory, nibble, poseidon2, program, public, range, sha256, F};
...
fn main() {
    ...
    let absorb_rows = exec.events.iter()
        .filter(|e| matches!(e.hash_row, Some(randprotocol_zkvm::emulator::HashRow::Absorb { .. })))
        .count();
    ...
    println!("same output, different private inputs: public values differ only in the salted H_IN = {}, proof bytes equal = {}", {
        ...
        diff == (randprotocol_zkvm::tables::cpu::pv::IN0..randprotocol_zkvm::tables::cpu::pv::IN0 + 8).collect::<Vec<_>>()
