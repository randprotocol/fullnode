...
fn poseidon2_syscall_matches_native_reference_for_various_lengths() {
    for n in [0usize, 1, 4, 5, 8, 100] {
        ...
        let e = run(&p, &[]);
        let want = randprotocol_zkvm::hash::sponge_hash(&msg);
...
fn poseidon2_of_the_empty_message_is_the_all_zero_digest() {
    assert_eq!(randprotocol_zkvm::hash::sponge_hash(&[]), [0u32; 8]);
...
fn read_public_returns_the_public_word_and_is_bound_by_its_own_length() {
    use randprotocol_zkvm::asm::{ops::*, Assembler};
    use randprotocol_zkvm::emulator::{execute, ExecError, Syscall};
    let mut a = Assembler::new(0);
