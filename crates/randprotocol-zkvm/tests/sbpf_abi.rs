...
use randprotocol_zkvm::notes;
use randprotocol_zkvm::sbpf::{self, asm, insn, lddw, Account, HostRef};
use sbpf_core::abi;
...
fn output_hash_covers_lamports_and_data_of_every_account() {
    ...
    assert_eq!(h0, randprotocol_zkvm::sha256::sha256(&msg));
    ...
    assert_eq!(abi::output_hash(&mut h, &buf), randprotocol_zkvm::sha256::sha256(&msg));
...
fn the_public_output_binds_the_input_and_the_post_state() {
    let mut h = HostRef;
    let input_hash = randprotocol_zkvm::sha256::sha256(b"an input");
    let out_hash = randprotocol_zkvm::sha256::sha256(b"a post-state");
...
fn expected_output_hash(entries: &[&Account]) -> [u8; 32] {
    ...
    randprotocol_zkvm::sha256::sha256(&msg)
}
...
fn an_spl_token_transfer_of_too_much_changes_nothing() {
    use randprotocol_zkvm::sbpf::{deserialize_accounts, spl_transfer};
    ...
    let (their_r0, their_region) =
        oracle::run_elf(randprotocol_zkvm::sbpf::SPL_TOKEN_ELF, &call.input);
...
fn the_sbpf_abi_reads_the_elf_from_the_public_segment() {
    ...
    let mut h = randprotocol_zkvm::sbpf::HostRef;
    let want = sbpf_core::abi::public_output(
        &mut h,
        1,
        &randprotocol_zkvm::sbpf::canonical_input_hash_of(&call.input),
        &randprotocol_zkvm::sbpf::output_hash_of(&call.input_post_state()),
    );
    assert_eq!(out, want);
}
