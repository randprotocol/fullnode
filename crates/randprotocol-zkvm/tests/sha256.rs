use randprotocol_zkvm::sha256::{bytes_to_words, compress, sha256, IV, K};
use sha2::Digest;
...
mod chip {
    ...
    use randprotocol_zkvm::tables::sha256::{self, col, sha256_trace, Sha256Event, BLOCK};
    ...
    fn sha256_trace_write_backs_match_the_reference_on_a_thousand_blocks() {
        ...
        for (i, e) in events.iter().enumerate() {
            ...
            let mut want = e.h_in;
            randprotocol_zkvm::sha256::compress(&mut want, &e.block);
            for k in 0..8 {
    ...
    fn sha256_padding_blocks_are_honest_zero_compressions_with_zero_counts() {
        ...
        let mut want = [0u32; 8];
        randprotocol_zkvm::sha256::compress(&mut want, &[0u32; 16]);
        for k in 0..8 {
...
mod harness {
    ...
    use randprotocol_zkvm::emulator::SPACE_RAM;
    use randprotocol_zkvm::machine::{make_config, FriProfile, VerifyError};
    use randprotocol_zkvm::tables::sha256::{col, sha256_trace, Sha256Air, Sha256Event, BLOCK};
    use randprotocol_zkvm::tables::{bus, F};
    ...
    fn twin_values(ev: &Sha256Event) -> Vec<F> {
        ...
        randprotocol_zkvm::sha256::compress(&mut h_out, &ev.block);
    ...
    fn the_chips_memory_sends_are_the_emulators_own_recorded_accesses() {
        ...
        use randprotocol_zkvm::emulator::{execute, Syscall};
        use randprotocol_zkvm::isa::REG_ZERO;
        use randprotocol_zkvm::sha256::IV;
