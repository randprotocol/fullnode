use randprotocol_zkvm::machine::{make_config, FriProfile};
use randprotocol_zkvm::tables::range::{self, range_trace, RangeAir, RangeCounts};
use randprotocol_zkvm::tables::nibble::{self, nibble_trace, NibbleCounts};
use randprotocol_zkvm::tables::bus;
...
use randprotocol_zkvm::isa::Instr;
use randprotocol_zkvm::tables::program::{self, fill_word_row, program_trace, ProgramAir};
use rand::distr::{Distribution, StandardUniform};
use rand::rngs::StdRng;
use rand::SeedableRng;
use randprotocol_zkvm::emulator::execute;
use randprotocol_zkvm::guests;
use randprotocol_zkvm::tables::memory::{self, memory_trace};
use randprotocol_zkvm::tables::alu::{self, fill_row};
use randprotocol_zkvm::emulator::AluEvent;
use randprotocol_zkvm::isa::AluOp;
...
fn program_decoder_equals_instr_decode() {
    ...
    let w = program::col::WIDTH;
    let height = randprotocol_zkvm::tables::pad_height(words.len(), 16);
...
use randprotocol_zkvm::tables::cpu::{self, cpu_trace, public_values};
...
fn input_digest_rows_len(inputs: &[u32]) -> usize { randprotocol_zkvm::hash::input_digest_rows([0u32; 4], inputs).len() }
...
fn input_table_shape_and_padding() {
    ...
    for i in 3..8 {
        let r = i * col::WIDTH;
        assert_eq!(t.values[r + col::IS_REAL], randprotocol_zkvm::tables::F::ZERO);
        assert_eq!(t.values[r + col::WORD], randprotocol_zkvm::tables::F::ZERO);
        assert_eq!(t.values[r + col::MULT_READ], randprotocol_zkvm::tables::F::ZERO);
        assert_eq!(t.values[r + col::IDX], randprotocol_zkvm::tables::F::from_u32(i as u32));
    }
}
...
fn cpu_trace_mirrors_events_and_pads() {
    ...
    let dr = p.digest_rows()
        + randprotocol_zkvm::hash::input_digest_row_count(0)
        + randprotocol_zkvm::hash::public_digest_row_count(0);
...
fn cpu_trace_limbs_and_counts_every_load_store_address() {
    ...
    let idr = randprotocol_zkvm::hash::input_digest_row_count(0);
    let pdr = randprotocol_zkvm::hash::public_digest_row_count(0);
...
/// 3`, i.e. `max_degree <= 8`).
///
/// `randprotocol_zkvm::machine::max_constraint_degrees` runs the *exact* computation
...
fn alu_max_constraint_degree_is_pinned() {
    use randprotocol_zkvm::machine::{max_constraint_degrees, Tier};
    use randprotocol_zkvm::tables::program::MIN_LOG_HEIGHT;
    ...
    let bare = max_constraint_degrees(
        Tier(10),
        MIN_LOG_HEIGHT,
        randprotocol_zkvm::tables::input::MIN_LOG_HEIGHT,
        0,
        0,
    ...
    let degrees = max_constraint_degrees(
        ...
        MIN_LOG_HEIGHT,
        randprotocol_zkvm::tables::input::MIN_LOG_HEIGHT,
        randprotocol_zkvm::tables::keccak::MIN_LOG_HEIGHT,
        randprotocol_zkvm::tables::sha256::MIN_LOG_HEIGHT,
        randprotocol_zkvm::tables::public::MIN_LOG_HEIGHT,
        Tier(10).min_mem_log_height(),
    );
    ...
    // `tables/sha256.rs`'s rule 12 records that measurement.)
    {
        ...
        use randprotocol_zkvm::machine::{Challenge, Chip, Machine};
        use randprotocol_zkvm::tables::sha256::{self, Sha256Air};
...
fn sha256_table_has_the_documented_width() {
    use randprotocol_zkvm::tables::sha256::{col, pre};
...
mod poseidon2_tests {
    ...
    use rand::rngs::StdRng;
    use rand::SeedableRng;
    use randprotocol_zkvm::machine::permutation;
    use randprotocol_zkvm::tables::poseidon2::{
...
mod keccak_tests {
    use super::*;
    use randprotocol_zkvm::keccak::RC;
    use randprotocol_zkvm::tables::keccak::{self, col, pre, KeccakAir, BLOCK, ROUNDS};
