...
//! `shrugg_getViewingNotes` call advances the cursor by at most [`MAX_SCAN_ROWS`] leaves, so one
//! request can never make the node re-walk unbounded history. The trial decryption itself is the
//! wallet's own logic (`shrugg_client::wallet::classify`) restated against a bare `ViewingKey`:
...
/// The most leaves one `shrugg_getViewingNotes` call trial-decrypts. Each costs an ML-KEM-768
...
/// `shrugg_zkvm::viewing::Role` a party's own key can produce.
...
/// `shrugg_client::wallet::classify`, which takes a whole wallet (a spend key) and so cannot be
/// reused here.
///
...
pub(crate) mod testkit {
    ...
    pub(crate) fn nk_hex(vk: &ViewingKey) -> String {
        shrugg_core::notes::word8_to_hex(&vk.nk)
    }
}
