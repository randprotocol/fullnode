...
use shrugg_node::rpc::{HeadSummary, NodeStatus, RpcState};
    ...
    Genesis {
        ...
        validators: vec![GenesisValidator {
            ...
            payout: shrugg_core::notes::ShieldedAddress {
                pk: [1; 8],
                kem_ek: vec![1; shrugg_core::notes::KEM_EK_BYTES],
            }
            .to_string(),
        }],
        alloc: vec![],
...
pub async fn serve_heads(capacity: usize) -> (SocketAddr, broadcast::Sender<HeadSummary>, ServedRpc) {
    ...
    let storage = Arc::new(shrugg_node::storage::Storage::open(dir.path()).unwrap());
