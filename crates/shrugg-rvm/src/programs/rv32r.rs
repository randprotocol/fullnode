...
//! interface list being the rVM's own: `[rvm_vk_digest ‖ N = 1 ‖ the proof's
//! `NUM_PUBLIC_VALUES` words]`. A tree of aggregates is this program over a tape of rVM
...
/// fresh challenger, then the interface digest over `[rvm_vk_digest ‖ 1 ‖ 4]`.
pub fn verify_rv32r(shape: &RvmShape, key: &RvmKey, cp: Checkpoints) -> VerifierProgram<RvmShape> {
    ...
    // (bound by the program digest twice over), then the list `[vk(4) ‖ N = 1 ‖ the proof's
    // four public values]`, sponged with the capacity header, the digest's four lanes
    // published — `rv32.rs`'s phase 8 construction, word for word.
    let vk = vk_digest_in_program(&mut b, shape, key);
    let n_list = DIGEST_ELEMS + 1 + npv;
