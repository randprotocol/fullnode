...
use shrugg_zkvm::machine::{FriProfile, Proof};
use shrugg_zkvm::tables::cpu::pv;
use shrugg_rvm::aggregate::{
...
use shrugg_rvm::machine::{Machine as RvmMachine, Tier as RvmTier};
use shrugg_rvm::programs::{aggregate_program_digest, verify_rv32, verify_rv32n};
...
fn n1_aggregate_publishes_the_single_proof_digest_at_a_pinned_overhead() {
    ...
    let words =
        shrugg_rvm::public_values::interface_words(&shape, &key, &[p.proof.public_values.clone()]);
    assert_eq!(
        exec.public,
        shrugg_rvm::public_values::public_digest(&words).to_vec(),
...
fn n3_aggregate_publishes_the_host_interface_digest() {
    ...
    let words = shrugg_rvm::public_values::interface_words(&shape, &key, &pvs);
    assert_eq!(
        exec.public,
        shrugg_rvm::public_values::public_digest(&words).to_vec(),
...
fn a_one_proof_aggregate_round_trips_and_tampered_variants_are_refused() {
    ...
    let proof2: shrugg_rvm::machine::Proof = postcard::from_bytes(&bytes).unwrap();
    ...
    let mut proof3: shrugg_rvm::machine::Proof = postcard::from_bytes(&bytes).unwrap();
...
fn a_tampered_tape_fails_the_prove_at_the_named_step() {
    ...
    match m.prove(&program.program, &tape.words, None) {
        Err(shrugg_rvm::machine::ProveError::Exec(ExecError::InverseOfZero { pc })) => {
            assert_eq!(
...
fn the_admission_stub_vectors() {
    let proofs: Vec<Proof> =
        common::bundle_proofs(FriProfile::Test, 3).into_iter().map(|p| p.proof).collect();
    let (shape, key) = shape_and_key(&proofs[0]);
    let vk_digest = shrugg_rvm::shape::inner_vk_digest(&shape, &key);
    assert_eq!(
        hex_words(&vk_digest),
    ...
    let pvs: Vec<Vec<u64>> = proofs.iter().map(|p| p.public_values.clone()).collect();
    let list = shrugg_rvm::public_values::interface_words(&shape, &key, &pvs);
    assert_eq!(list.len(), 4 + 1 + 34 * 3);
    let digest = shrugg_rvm::public_values::public_digest(&list);
