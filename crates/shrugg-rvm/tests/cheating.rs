...
use shrugg_rvm::machine::{build_traces, FriProfile, Machine, Tier, Traces};
use shrugg_rvm::tables::{cpu, memory, poseidon2, program as program_table, public as public_table};
...
fn prove_and_verify(m: &Machine, p: &Program, t: &Traces) -> Result<(), shrugg_rvm::machine::VerifyError> {
...
fn an_out_of_range_tier_is_an_error_not_a_panic() {
    ...
    let mut proof = m.prove_traces(&p, &t, Tier(8));
    proof.tier = Tier(99);
    assert!(matches!(m.verify(&p, &proof), Err(shrugg_rvm::machine::VerifyError::Tier)));
}

// ── Task 8: the reduce chip's tranche ─────────────────────────────────────────────────────────
use p3_field::BasedVectorSpace;
use shrugg_rvm::isa::EF;
use shrugg_rvm::tables::reduce as reduce_table;
...
fn reduce_verify(m: &Machine, p: &Program, t: &Traces) -> Result<(), shrugg_rvm::machine::VerifyError> {
    let proof = m.prove_traces(p, t, Tier(8));
    m.verify(p, &proof)
}
...
fn a_reduce_proof_declaring_no_table_is_rejected() {
    ...
    assert!(matches!(
        m.verify(&p, &proof),
        Err(shrugg_rvm::machine::VerifyError::Tier)
    ));
}
...
fn sponge_setup() -> (Machine, Program, Traces) {
    use shrugg_rvm::dsl::{Builder, Checkpoints, Digest, Liveness};
    ...
    shrugg_rvm::dsl::hash::sponge(&mut b, src, 4, out);
    for k in 0..4 {
