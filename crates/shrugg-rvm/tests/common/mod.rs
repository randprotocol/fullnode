...
//!
//! Built from `shrugg_zkvm`'s public API the way `research/tests/bundle.rs::fixture()` does — two
...
//! from `Machine::prove`.
use shrugg_zkvm::ledger::Ledger;
use shrugg_zkvm::machine::{FriProfile, Machine, Proof};
use shrugg_zkvm::notes::{self, Note, SpendKey, ViewingKey, Word8, DEPTH};
use shrugg_zkvm::viewing::{Envelope, TxKey};
...
#[allow(dead_code)]
pub fn measure_production_inner_proof() -> shrugg_rvm::programs::CycleReport {
    ...
    let exec = shrugg_rvm::emulator::execute(&vp.program, &tape.words, 1 << 24).unwrap();
    shrugg_rvm::programs::cycle_report(&vp, &exec)
}
...
#[allow(dead_code)]
pub fn measure_aggregate(n: usize, profile: FriProfile) -> shrugg_rvm::programs::CycleReport {
    use shrugg_rvm::dsl::Checkpoints;
    use shrugg_rvm::programs::verify_rv32n;
    use shrugg_rvm::shape::{InnerKey, InnerShape};
    use shrugg_rvm::witness::WitnessTape;
...
pub fn committed_digest() -> String {
    ...
    let key = InnerKey::of(FriProfile::Production, &shape);
    let vp = verify_rv32(&shape, &key, Checkpoints::Off);
    let hex = shrugg_rvm::programs::digest_hex(&vp.program);
    std::fs::write(&path, format!("{hex}\n")).expect("the digest file is writable");
    hex
}

#[allow(dead_code)]
pub fn random_felt(rng: &mut impl rand::Rng) -> shrugg_rvm::isa::F {
    use p3_field::{PrimeCharacteristicRing, PrimeField64};
    use rand::RngExt;
    shrugg_rvm::isa::F::from_u64(rng.random::<u64>() % shrugg_rvm::isa::F::ORDER_U64)
}

#[allow(dead_code)]
pub fn random_ext(rng: &mut impl rand::Rng) -> shrugg_rvm::isa::EF {
    use p3_field::BasedVectorSpace;
    let c = [random_felt(rng), random_felt(rng)];
    shrugg_rvm::isa::EF::from_basis_coefficients_slice(&c).expect("an extension element is two coefficients")
}
...
#[allow(dead_code)]
pub fn rejects(f: impl FnOnce() -> Result<(), shrugg_rvm::machine::VerifyError>) -> bool {
