...
use shrugg_rvm::emulator::ExecError;
use shrugg_rvm::isa::{F, Instr, Op, Program};
use shrugg_rvm::machine::{FriProfile, Machine, ProveError, Tier};
...
fn prove_and_verify(p: &Program, w: &[F]) -> (shrugg_rvm::machine::Proof, shrugg_rvm::emulator::Execution) {
...
fn contract_program(states: &[[F; 8]]) -> (Program, Vec<F>) {
    ...
    for state in states {
        ...
        instrs.push(i(Op::Poseidon2, 0, 1, 0));
        let want = shrugg_zkvm::hash::permute_state(*state);
        for k in 0..8u64 {
            instrs.push(i(Op::Load, 3, 1, k));
