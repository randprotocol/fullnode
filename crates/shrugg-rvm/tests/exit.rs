...
use shrugg_rvm::programs::{cycle_report, verify_rv32};
use shrugg_rvm::shape::{InnerKey, InnerShape};
use shrugg_rvm::witness::{Segment, WitnessTape};
    ...
    match seg {
        ...
        Segment::CommitPhaseOpenings => {
            ...
            let strides: Vec<usize> = shape
                .log_arities
                .iter()
                .map(|&la| ((1usize << la) - 1) * 2 + shrugg_rvm::witness::SALT_ELEMS)
                .collect();
...
fn the_cycle_budget_per_inner_proof_is_pinned() {
    ...
    assert_eq!(shrugg_rvm::programs::digest_hex(&vp.program),
            ...
            .filter_map(|(i, ins)| {
                ...
                match ins.op {
                    Op::Load | Op::Loade | Op::Store | Op::Storee
                        if ins.ra == 0 && ins.b.as_canonical_u64() < shrugg_rvm::dsl::MEM_BASE =>
                    {
                        None
                    }
                    _ => {
...
fn twin_the_post_cut_verifier_program_over_one_test_profile_proof_proves_and_verifies_natively() {
    ...
    assert_eq!(rvm_proof.tier, shrugg_rvm::machine::Tier(19));
    ...
    other_vp.program.instrs[123] = shrugg_rvm::isa::Instr { op: shrugg_rvm::isa::Op::Faddi, rd: 9, ra: 0, b: shrugg_rvm::isa::F::from_u64(1) };
...
fn exit_the_verifier_program_over_one_real_cs6_bundle_proof_proves_and_verifies_natively() {
    ...
    let key = InnerKey::of(FriProfile::Production, &shape);
    let vp = verify_rv32(&shape, &key, Checkpoints::Off);
    assert_eq!(shrugg_rvm::programs::digest_hex(&vp.program), common::committed_digest(),
               "the registered program is the committed one");
    let tape = WitnessTape::build(FriProfile::Production, &shape, &key, &p.proof).unwrap();
    let m = shrugg_rvm::machine::Machine::new(FriProfile::Production);
    let t0 = std::time::Instant::now();
    let (rvm_proof, exec) = m.prove(&vp.program, &tape.words, None).unwrap();
    let prove_s = t0.elapsed().as_secs_f64();
    assert_eq!(exec.cpu_rows(), 1_968_619, "the exit proves the post-cut program as measured");
    assert_eq!(rvm_proof.tier, shrugg_rvm::machine::Tier(21));
    let t1 = std::time::Instant::now();
    m.verify(&vp.program, &rvm_proof).unwrap();
    ...
    assert!(
        matches!(m.prove(&vp.program, &bad_tape, None), Err(shrugg_rvm::machine::ProveError::Exec(_))),
