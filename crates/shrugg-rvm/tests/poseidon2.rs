...
use shrugg_rvm::emulator::PermEvent;
use shrugg_rvm::isa::F;
use shrugg_rvm::tables::poseidon2::{col, poseidon2_log_height, poseidon2_trace, ROUNDS_F, ROUNDS_P};

fn event(ptr: u64, input: [F; 8]) -> PermEvent {
    PermEvent { ptr, input, output: shrugg_zkvm::hash::permute_state(input), src: None }
}
...
fn the_chips_output_equals_the_reference_on_one_thousand_random_states() {
    ...
    for (i, &(clk, ev)) in events.iter().enumerate() {
        ...
        assert_eq!(output, shrugg_zkvm::tables::poseidon2::permute_scalar(ev.input),
    ...
    assert_eq!(out, shrugg_zkvm::tables::poseidon2::permute_scalar([F::ZERO; 8]));
...
fn the_round_constants_are_the_machines_own_draw() {
    ...
    // shares all come from `shrugg_zkvm::tables::poseidon2`.
    let rc = shrugg_zkvm::tables::poseidon2::round_constants();
    ...
    for _ in 0..20 {
        ...
        assert_eq!(
            shrugg_zkvm::tables::poseidon2::permute_scalar(s),
            shrugg_zkvm::hash::permute_state(s),
