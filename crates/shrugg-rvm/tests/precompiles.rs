...
use shrugg_rvm::dsl::{Array, Builder, Checkpoints, Ext, Felt};
use shrugg_rvm::emulator::{execute, ExecError};
use shrugg_rvm::isa::{EF, F};
use shrugg_rvm::programs::{reduce_compiled, run_reduce_sequence};
...
use shrugg_rvm::machine::{FriProfile, Machine};
...
fn two_run_program(vals1: &[EF], row1: &[F], vals2: &[EF], row2: &[F], inv: EF, alpha: EF) -> (shrugg_rvm::isa::Program, Vec<F>) {
    let mut b = Builder::new(Checkpoints::Off);
    let mut tape: Vec<F> = vec![];
...
fn sponge_via_the_precompile_matches_padding_free_sponge() {
    use shrugg_rvm::programs::Precompiles;
    let perm = shrugg_zkvm::machine::permutation();
    let sponge = p3_symmetric::PaddingFreeSponge::<shrugg_zkvm::machine::Perm, 8, 4, 4>::new(perm);
    ...
    for n in [1usize, 3, 4, 5, 8, 9, 37, 121] {
        ...
        shrugg_rvm::dsl::hash::sponge(&mut b, src, n, out);
        for k in 0..4 {
