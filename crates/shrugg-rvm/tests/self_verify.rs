...
use shrugg_rvm::machine::{FriProfile, Machine, Tier};
use shrugg_rvm::programs::{self_program_digest, verify_rv32, verify_rv32r};
use shrugg_rvm::shape::{InnerKey, InnerShape, RvmKey, RvmShape, VerifierShape};
...
fn fixture() -> (Arc<Program>, shrugg_rvm::machine::Proof, RvmShape, RvmKey) {
...
fn the_self_verifier_accepts_a_real_rvm_proof() {
    ...
    let words =
        shrugg_rvm::public_values::interface_words(&shape, &key, &[proof.public_values.clone()]);
    assert_eq!(
        exec.public,
...
fn a_wrong_shape_proof_is_refused() {
    ...
    assert_eq!(err, shrugg_rvm::witness::TapeError::Replay(shrugg_rvm::reference::ReplayError::Shape));
}
...
fn busy_fixture() -> (Arc<Program>, shrugg_rvm::machine::Proof, RvmShape, RvmKey) {
...
fn the_self_verifiers_measured_cost_at_two_fixture_shapes() {
    ...
    let r = shrugg_rvm::programs::cycle_report(&vp, &exec);
    ...
    let rb = shrugg_rvm::programs::cycle_report(&vp_b, &exec_b);
    eprintln!("BUSY {rb:?}");
