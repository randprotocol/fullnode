//! The verifier program, against a real `shrugg_zkvm` bundle proof.
//!
...
use shrugg_rvm::dsl::Checkpoints;
use shrugg_rvm::emulator::{execute, ExecError};
use shrugg_rvm::isa::F;
use shrugg_rvm::programs::verify_rv32;
use shrugg_rvm::reference::replay;
use shrugg_rvm::shape::{InnerKey, InnerShape};
use shrugg_rvm::witness::WitnessTape;
...
fn the_witness_tape_layout_is_pinned() {
    ...
    use shrugg_rvm::witness::Segment::*;
    assert_eq!(
        names,
        vec![
            Header,
...
fn the_host_transcript_replay_reproduces_machine_verifys_acceptance() {
    ...
    assert_eq!(
        ...
        shape.log_arities.iter().sum::<usize>() + shrugg_rvm::shape::LOG_BLOWUP
    );
...
fn the_program_reproduces_the_lookup_challenges_alpha_and_zeta() {
    ...
    let cp = shrugg_rvm::programs::checkpoint_values(&vp, &exec);
...
fn a_tampered_lookup_terminal_is_refused_at_the_terminal_sum_checkpoint() {
    ...
    let (_, start, len) = *tape
        .segments
        .iter()
        .find(|(s, _, _)| *s == shrugg_rvm::witness::Segment::LookupTerminals)
        .unwrap();
...
fn the_opened_value_segments_are_sized_by_the_shape_alone() {
    let (p, shape, key) = one_test_proof();
    let tape = WitnessTape::build(FriProfile::Test, &shape, &key, &p.proof).unwrap();
    let seg = |s: shrugg_rvm::witness::Segment| {
    ...
    assert_eq!(seg(shrugg_rvm::witness::Segment::OpenedValues), 2 * opened);
    assert_eq!(seg(shrugg_rvm::witness::Segment::RandomOpenings), 2 * 4 * random);
}
...
fn the_query_bit_segment_decodes_to_the_sampled_query_indices() {
    ...
    let (_, start, len) = *tape
        .segments
        .iter()
        .find(|(s, _, _)| *s == shrugg_rvm::witness::Segment::QueryBits)
        .unwrap();
    ...
    assert_eq!(hint, shrugg_rvm::dsl::transcript::canonicality_hint(pow));
    ...
    for q in 0..shape.num_queries {
        ...
        assert_eq!(hint, shrugg_rvm::dsl::transcript::canonicality_hint(v), "query {q}");
...
fn the_query_segments_are_sized_by_the_round_geometry() {
    ...
    use shrugg_rvm::dsl::DIGEST_ELEMS;
    use shrugg_rvm::witness::{open_stride, path_stride, per_query_levels, per_query_rows, Segment};
...
fn a_commit_phase_leaf_and_its_restored_path_recompute_the_rounds_commitment() {
    ...
    use shrugg_rvm::dsl::{hash, Builder, Digest, Felt};
    use shrugg_rvm::isa::EF;

    let (p, shape, key) = one_test_proof();
    let r = replay(FriProfile::Test, &shape, &key, &p.proof).unwrap();
    let tape = WitnessTape::build(FriProfile::Test, &shape, &key, &p.proof).unwrap();
    let at = |s: shrugg_rvm::witness::Segment| {
        tape.segments.iter().find(|(x, _, _)| *x == s).unwrap().1
    };
    let opens = at(shrugg_rvm::witness::Segment::CommitPhaseOpenings);
    let paths = at(shrugg_rvm::witness::Segment::CommitPhasePaths);
    ...
    let levels = (r.log_global_max_height - log_arity) - shrugg_rvm::shape::CAP_HEIGHT;
    let open_stride = shrugg_rvm::witness::open_stride(&r.log_arities);
    let path_stride = shrugg_rvm::witness::path_stride(r.log_global_max_height, &r.log_arities);
    ...
    for q in 0..shape.num_queries {
        ...
        msg.extend_from_slice(&tape.words[salt_at..salt_at + shrugg_rvm::witness::SALT_ELEMS]);
...
fn an_input_rounds_leaf_group_and_restored_path_recompute_the_preprocessed_cap() {
    use shrugg_rvm::dsl::{hash, Builder, Digest, Felt};
    use shrugg_rvm::witness::levels_for;
    ...
    let opens = tape
        .segments
        .iter()
        .find(|(s, _, _)| *s == shrugg_rvm::witness::Segment::InputOpenings)
        .unwrap()
        .1;
    let paths = tape
        .segments
        .iter()
        .find(|(s, _, _)| *s == shrugg_rvm::witness::Segment::InputPaths)
        .unwrap()
        .1;
    ...
    let words_of = |m: usize| dims[m].width + shrugg_rvm::witness::SALT_ELEMS;
    ...
    use shrugg_rvm::dsl::DIGEST_ELEMS;
    use shrugg_rvm::witness::{per_query_levels, per_query_rows};
    let round_at =
...
fn a_header_word_that_disagrees_with_the_programs_shape_is_refused_at_that_word() {
    ...
    let vp = verify_rv32(&shape, &key, Checkpoints::Off);
    let tape = WitnessTape::build(FriProfile::Test, &shape, &key, &p.proof).unwrap();
    let (_, start, len) = *tape.segments.iter().find(|(s, _, _)| *s == shrugg_rvm::witness::Segment::Header).unwrap();
...
fn a_tampered_main_commitment_diverges_at_the_zeta_checkpoint() {
    ...
    let (_, start, _) = *tape
        .segments
        .iter()
        .find(|(s, _, _)| *s == shrugg_rvm::witness::Segment::Commitments)
        .unwrap();
    ...
    match exec {
        Ok(e) => {
            ...
            let cp = shrugg_rvm::programs::checkpoint_values(&vp, &e);
...
fn the_quotient_identity_holds_in_the_program_for_a_real_proof() {
    ...
    let words = shrugg_rvm::public_values::interface_words(&shape, &key, &[p.proof.public_values.clone()]);
    assert_eq!(exec.public, shrugg_rvm::public_values::public_digest(&words).to_vec(),
...
fn a_tampered_opened_value_fails_the_quotient_identity_at_the_named_checkpoint() {
    ...
    let (_, start, _) = *tape
        .segments
        .iter()
        .find(|(s, _, _)| *s == shrugg_rvm::witness::Segment::OpenedValues)
        .unwrap();
...
fn phase_5_costs_the_measured_number_of_rows_per_inner_proof() {
    ...
    let sum = |f: fn(&shrugg_rvm::programs::constraints::Phase5Cost) -> usize| -> usize {
...
fn the_off_replay_reproduces_the_pre_liveness_program_byte_for_byte() {
    use shrugg_rvm::dsl::Liveness;
    use shrugg_rvm::programs::verify_rv32_with;
    ...
    let off = verify_rv32_with(&shape, &key, Checkpoints::Off, Liveness::Off, shrugg_rvm::programs::Precompiles::Off);
    assert_eq!(
        shrugg_rvm::programs::digest_hex(&off.program),
    ...
    let on = verify_rv32_with(&shape, &key, Checkpoints::Off, Liveness::On, shrugg_rvm::programs::Precompiles::On);
