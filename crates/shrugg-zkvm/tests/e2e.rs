...
fn sha256_demo_proves_and_verifies_with_one_sha256_block() {
    ...
    let want = shrugg_zkvm::sha256::sha256(msg);
    for k in 0..8 {
        assert_eq!(
            exec.outputs[k],
...
fn compiled_evm_erc20_transfer_binds_the_state_root_transition() {
    ...
    use shrugg_zkvm::emulator::execute;
    use shrugg_zkvm::evm::{erc20_transfer, ALICE, BOB};
    use shrugg_zkvm::hash::input_digest_row_count;
    ...
    let mut h = shrugg_zkvm::evm::HostRef;
    assert_eq!(
...
fn compiled_evm_storage_read_write_and_return_proves_and_verifies() {
    use evm_core::u256::U256;
    use shrugg_zkvm::evm::{EvmCall, SparseTree};
    let m = Machine::new(FriProfile::Test);
    let p = guests::compiled::evm();
...
fn compiled_sbpf_spl_token_transfer_executes_and_publishes_the_bound_digest() {
    use shrugg_zkvm::sbpf::{deserialize_accounts, spl_transfer, TOKEN_AMOUNT_AT};
    ...
    let exec = shrugg_zkvm::emulator::execute(&p, &inputs, &public, CYCLE_CAP).unwrap();
    ...
    assert_eq!(
        shrugg_zkvm::tables::sha256::sha256_log_height(compressions),
        11,
    ...
    let native = shrugg_zkvm::sbpf::run_elf(&mut call.elf.clone(), &mut call.input.clone());
...
fn compiled_sbpf_spl_token_transfer_of_too_much_fails_cleanly() {
    let p = guests::compiled::sbpf();
    let call = shrugg_zkvm::sbpf::spl_transfer(u64::MAX / 2);
    let (want, r0, post) = call.expected();
    assert!(matches!(r0, Ok(code) if code != 0));
    assert_eq!(want[0], 0);
    assert_eq!(post, shrugg_zkvm::sbpf::deserialize_accounts(&call.input));
    let exec =
        shrugg_zkvm::emulator::execute(&p, &call.input_words(), &call.public_words(), 4_000_000)
            .unwrap();
    ...
    let ok = shrugg_zkvm::sbpf::spl_transfer(250).expected().0;
...
fn compiled_sbpf_spl_token_transfer_proves_and_verifies() {
    let m = Machine::new(FriProfile::Test);
    let p = guests::compiled::sbpf();
    let call = shrugg_zkvm::sbpf::spl_transfer(250);
    let private = call.input_words();
    let public = call.public_words();
    let (want, _r0, _post) = call.expected();
    let exec =
        shrugg_zkvm::emulator::execute(&p, &private, &public, Tier(20).max_cycles()).unwrap();
    assert_eq!(exec.outputs, want);
    let t0 = std::time::Instant::now();
...
fn public_echo_proves_and_verify_public_checks_the_words() {
    ...
    let want = shrugg_zkvm::hash::public_digest(&public);
    for i in 0..8 {
        assert_eq!(proof.public_values[shrugg_zkvm::tables::cpu::pv::PUB0 + i], want[i] as u64);
    }
    ...
    assert_eq!(proof.public_log_height, shrugg_zkvm::tables::public::public_log_height(4));
}
...
fn an_empty_public_segment_still_has_a_digest_and_costs_four_rows() {
    ...
    assert_eq!(proof.public_log_height, shrugg_zkvm::tables::public::MIN_LOG_HEIGHT);
    let want = shrugg_zkvm::hash::public_digest(&[]);
    for i in 0..8 {
        assert_eq!(proof.public_values[shrugg_zkvm::tables::cpu::pv::PUB0 + i], want[i] as u64);
    }
}
