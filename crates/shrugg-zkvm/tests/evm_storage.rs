//! The EVM contract storage tree (M4.3 Task 2): the host builder (`shrugg_zkvm::evm`) and the
...
use shrugg_zkvm::evm::{empty_root, leaf_hash, slot_index, HostRef, SparseTree, Witness};
use shrugg_zkvm::notes::domain;
...
fn the_guest_crates_keccak256_and_dhash_match_the_research_reference() {
    ...
    for len in [0usize, 1, 135, 136, 137, 272] {
        ...
        assert_eq!(evm_core::keccak256(&mut h, &msg), shrugg_zkvm::keccak::keccak256(&msg), "keccak256 of {len} bytes");
    }
    for n in [0usize, 1, 8, 16, 40] {
        let msg: Vec<u32> = (0..n as u32).map(|i| i.wrapping_mul(2_654_435_761)).collect();
        assert_eq!(evm_core::dhash(&mut h, domain::STORAGE_LEAF, &msg), shrugg_zkvm::notes::hash(domain::STORAGE_LEAF, &msg), "dhash of {n} words");
    }
}
...
fn host_and_guest_hash_the_same_leaves_indices_and_roots() {
    ...
    let k = shrugg_zkvm::keccak::keccak256(&s(1).to_be_bytes());
...
fn the_host_refuses_to_build_a_vector_with_two_witnesses_at_one_position() {
    ...
    let call = shrugg_zkvm::evm::EvmCall {
        code: vec![0x00],
        calldata: vec![],
        address: U256::ZERO,
        caller: U256::ZERO,
...
fn the_guest_fold_is_the_note_trees_index_convention() {
    ...
    let leaves: Vec<[u32; 8]> = (0..5u32).map(|i| shrugg_zkvm::notes::hash(domain::TEST, &[i])).collect();
