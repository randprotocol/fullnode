...
fn the_data_segment_is_in_ram_before_the_guest_entry_runs() {
    ...
    let exec = shrugg_zkvm::emulator::execute(&p, &[], &[], Tier(12).max_cycles()).unwrap();
    ...
    let m = shrugg_zkvm::machine::Machine::new(shrugg_zkvm::machine::FriProfile::Test);
...
fn the_prologue_repoints_its_base_register_past_the_immediate_range() {
    ...
    let exec = shrugg_zkvm::emulator::execute(&p, &[], &[], Tier(14).max_cycles()).unwrap();
...
fn public_digest_is_the_unsalted_twin_of_the_program_digest() {
    use shrugg_zkvm::hash::{public_digest, public_digest_row_count, public_digest_rows};
    ...
    assert_ne!(public_digest(&[1, 2, 3, 4]), shrugg_zkvm::hash::input_digest([0; 4], &[1, 2, 3, 4]));
    ...
    assert_eq!(shrugg_zkvm::hash::split_digest([blocks[1].state_out[0], blocks[1].state_out[1], blocks[1].state_out[2], blocks[1].state_out[3]]), public_digest(&[1, 2, 3, 4, 5]));
}
