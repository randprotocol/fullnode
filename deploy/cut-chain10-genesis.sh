#!/usr/bin/env bash
# Cut the chain-10 genesis: chain 9's eighteen validators and five deposit notes, on the RAND-named
# build — every hash domain, the address prefix (`rand1`), the transaction id and the p2p
# identity changed with the rename, so chain 10 is a fresh chain like every rename before it.
# Cut WITHOUT the aggregation section by default (AGGREGATION=on restores chain 9's script
# behaviour; the section's activation values are still the user-owned ≥ 64 GB measurements).
#
# The regional validators' public keys come from the chain-9 genesis (positions 6..17); the six
# local nodes from their key files; every payout from deploy/payout/<name>.key.json (the same
# keys — only the text form of the address changed prefix).
#
...
CHAIN9=${CHAIN9:-deploy/genesis-chain9.json}
ALLOC_WALLETS=${ALLOC_WALLETS:-wallets}          # shielded-{1..5}.key.json (gitignored)
OUT=${OUT:-deploy/genesis-chain10.json}
...
for i in "${!regions[@]}"; do
  pk=$(python3 -c "import json,sys;print(json.load(open(sys.argv[1]))['validators'][6+int(sys.argv[2])]['public_key'])" "$CHAIN9" "$i")
...
if [ "$AGGREGATION" = off ]; then
  "$NODE" genesis --chain-id "${CHAIN_ID:-10}" "${args[@]}" \
