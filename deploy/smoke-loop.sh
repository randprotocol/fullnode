#!/usr/bin/env bash
# Health loop for the shielded testnet (chain 8): every INTERVAL seconds run one round of
#   1. mint (faucet, validator-signed, no bundle)
#   2. RAND transfer (a proved 2-in-2-out bundle, ~100 s on a laptop)
...
NODES=${NODES:-"c=164.90.239.200 d=165.245.173.74 e=188.166.235.187 f=159.89.185.254 lon1=139.59.160.76 sfo3=24.144.89.22 tor1=146.190.243.29 blr1=167.71.235.108 syd1=170.64.226.65 atl1=165.245.142.90 ams3=146.190.233.230 nyc1=192.81.214.91 nyc2=107.170.49.234 sfo2=143.110.135.126 mkc1=201.79.35.212 mem1=168.144.61.10"}
...
while true; do
  ...
  log "round $round start (chain $(rpc rand_chainId '[]' | grep -o '"result":[0-9]*' | cut -d: -f2), head $(rpc rand_getHead '[]' | grep -o '"height":[0-9]*' | cut -d: -f2))"

  # 1. mint
  out=$("$BIN/rand" faucet --key "$W1" --rpc "$RPC" --amount 10 2>&1); h=$(submitted "$out" mint)
  ...
  "$BIN/rand" program build --guest private_payment --arg $((100 + round)) --out "$pj" >/dev/null 2>&1
  t0=$(date +%s); out=$("$BIN/rand" program deploy "$pj" --key "$W1" --rpc "$RPC" 2>&1); h=$(submitted "$out" deploy)
  ...
  if [ -n "$pid" ]; then
    t0=$(date +%s); out=$("$BIN/rand" call "$pid" --input 400 --input 250 --input 0 --input 0 --key "$W1" --rpc "$RPC" 2>&1); h=$(submitted "$out" call)
  ...
  local_h=$(rpc rand_getHead '[]' | grep -o '"height":[0-9]*' | cut -d: -f2)
  heights="a=$local_h"
  for n in $NODES; do
    ip=${n#*=}; hh=""
    # Two tries with a generous timeout: a slow SSH handshake to a far region is not a down node.
    for try in 1 2; do
      hh=$(ssh -n -i "$KEY" -o ConnectTimeout=25 -o BatchMode=yes -o StrictHostKeyChecking=accept-new "root@$ip" 'rand status 2>/dev/null | grep -o "\"height\": *[0-9]*" | grep -o "[0-9]*$"' 2>/dev/null)
      [ -n "$hh" ] && break; sleep 3
    done
    heights="$heights ${n%%=*}=${hh:-down}"
  done
  ex=$(curl -s --max-time 10 "$EXPLORER/health")
  log "round $round health heights [$heights] explorer $(grep -o '"status":"[a-z]*"' <<<"$ex") lag $(grep -o '"lag":[0-9-]*' <<<"$ex" | cut -d: -f2) balance $("$BIN/rand" balance --key "$W1" --rpc "$RPC" 2>/dev/null | grep -o 'balance: .*')"
