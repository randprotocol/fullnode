...
ExecStart=/usr/local/bin/rand-node run --datadir $DATA --key /root/fullnode/deploy/node-$NODE.key.json $VALIDATOR_FLAG --listen /ip4/0.0.0.0/tcp/30303 --rpc 127.0.0.1:8545 --no-mdns $BOOT_ARGS
Restart=always
RestartSec=3
[Install]
