...
Action = None | Mint { .. } | Deploy { base_pc, words } | Call { program, proof, input_envelope }
       | Bond { .. } | Unbond { .. } | Withdraw { .. }                    // phase S2, scaffolded
...
3. **Shape and fee floor** — a mint carries no bundle, everything else must; the transaction's own
   bundle is always SHRUGG (`asset = 0`) and never burns (`burn = 0`), a rule only a `BridgeBurn`'s
   second bundle is exempt from; `fee ≥ fee_floor(action)`, which for a burn is the bundle base
   twice, once per bundle a node has to verify.
...
7. **Action checks** — faucet enabled, mint under the 100 SHRUGG cap, minter is a validator and
   ...
   within its own cap (Call); the bridge's own rules for the two bridge actions, including a
   burn's asset bundle in full — all of it before either bundle's proof, so a bridge transaction
   that cannot apply costs no verification (`docs/bridge.md` §5).
...
S1 is the pool itself. Two phases follow it, each a hard fork (see spec §12); S3 has landed and S2
is still ahead — the order they arrived in, not the order they were planned in:
