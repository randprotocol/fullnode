...
3. Maintains an account-based ledger of the native token **RAND** and applies signed transfers.
...
fullnode/
  ...
  crates/
    randprotocol-core/               pure, no async, no I/O
...
- libp2p peer identity: ed25519 key derived as `blake3("rand-p2p-identity" || seed)`;
...
```
Balances are in the smallest unit; 1 RAND = 10^9 units (`RAND_DECIMALS = 9`).
...
Pure state machine `HotStuff` in `randprotocol-core::consensus`, driven by the node
...
`randprotocol_core::confidential` defines
...
confidential computation. `rand_sendTransaction` accepts both kinds.
...
- Topics: `rand/consensus/1`, `rand/tx/1`.
...
|---|---|---|
| `rand_chainId` | | `u64` |
| `rand_getBalance` | `[address]` | `u128` string |
| `rand_getAccount` | `[address]` | `{nonce, balance}` |
| `rand_sendTransaction` | `[hex(bincode(tx))]` | tx hash |
| `rand_getTransaction` | `[hash]` | `{tx, height, index}` or null |
| `rand_getBlockByHeight` | `[height]` | block or null |
| `rand_getBlockByHash` | `[hash]` | block or null |
| `rand_getHead` | | `{height, hash, view}` |
| `rand_getPeers` | | `[{peer_id, addrs}]` |
| `rand_getValidators` | | `[{address, stake}]` |
| `rand_syncStatus` | | `{syncing, head, target}` |

## CLI

```
rand-node keygen --out key.json
rand-node init --genesis genesis.json --datadir ./data
rand-node run --datadir ./data --key key.json --listen /ip4/0.0.0.0/tcp/30303 \
              --rpc 127.0.0.1:8545 [--bootstrap <multiaddr>]... [--validator]
rand-node balance --rpc http://127.0.0.1:8545 <address>
rand-node transfer --rpc http://127.0.0.1:8545 --key key.json --to <address> --amount 1.5
rand-node genesis-template --validators key1.json,key2.json > genesis.json
```
...
- Integration test (`crates/randprotocol-node/tests/cluster.rs`): 4 validators in-process on
...
1. Token symbol `RAND` per the request, even though whitepaper Draft 3 renamed it RAND. One constant.
