...
## 6. M4 — compatibility (M4.1 and M4.2 shipped; the EVM/sBPF interpreters are still open)
...
the eighth (`input`) table. **M4.2 followed on 2026-09-12** (constraint set 5): the `keccak` table
and `KECCAK` syscall — the hand-written Keccak-f[1600] coprocessor chip this section's next
paragraph says M4 would need — made *optional per proof* (`keccak_log_height = 0` means the batch
has eight instances and no keccak table, which is every proof this chain sees), plus
proof-declared `mem_log_height` and a four-keyed verifier key. What remains open is the
interpreters themselves: neither the EVM nor sBPF ever executes natively — each
...
a Keccak coprocessor chip had to be hand-written the same way `tables/poseidon2.rs` was, not
imported — which is exactly what M4.2 did (`tables/keccak.rs`, 2 612 columns, 32-row blocks,
checked against `p3_keccak` on 1 000 random states). Two other facts bear on M4 without a design existing yet: `solana-sbpf` 0.11.1 is
